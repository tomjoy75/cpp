...
template<typename Container>
Container create_s(std::vector<Container >& sorted_split_array, int stash, bool print_comparison_estimation) {
    Container S, pend;
...
Container merge_insertion_sort(Container A) {
    ...
    Container S = create_s(sorted_split_array, stash, true);
    return S;
}
